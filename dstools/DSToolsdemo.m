...
% to compute the separation of proper and polynomial parts of Gc(s), we can use

[sysf,sysi] = gsdec(sysc);

% for checking the results, convert the terms to zeros/poles/gain form

zpk(sysf)   % proper part

zpk(sysi)   % polynomial part
...
% having a stability degree -1 and poles assigned to [-1,-2,-3], we can use

[sysn,sysm] = grcf(sysc,struct('poles',[-1,-2,-3,-4],'sdeg',-1));

pause % Press any key to continue ...

% checking the factorization 

norm(gminreal(sysc*sysm-sysn),inf)  % ||Gc(s)*M(s)-N(s)||_inf = 0

pause % Press any key to continue ...

[gpole(sysm), gpole(sysn)]         % checking poles
...
norm(gminreal(sysd*sysmi-sysni),inf)     %  ||Gd(z)*M(z)-N(z)||_inf = 0

pause % Press any key to continue ...

% checking the innerness of M(z)

norm(gminreal(sysmi'*sysmi-eye(2)),inf)  % ||conj(M(z))*M(z)-I||_inf = 0

pause % Press any key to continue ...

[gpole(sysmi), gpole(sysni)]             % checking the stability of poles
...
norm(sysi(:,1:nr)*syso-sys,inf)   %  ||Gi(:,1:nr)(s)*Go(s)-G(s)||_inf = 0

pause % Press any key to continue ...

% checking the innerness of Gi(s)
norm(sysi'*sysi-eye(3),inf)        %  ||conj(Gi(s))*Gi(s)-I||_inf = 0
...
gzero(syso)       % check that there are no zeros in open right-half plane 
pause % Press any key to continue ...
...
norm(sys*sysx-F,inf)   %  ||G(s)*X(s) - I||_inf = 0
...
% checking the solution 
norm(sys*sysxmin-F,inf)   %  ||G(s)*X(s) - I||_inf = 0
...
% checking the solution 
norm(sys*sysx2-F,inf)   %  ||G(s)*X2(s) - I||_inf = 0
...
norm(gminreal(sysx2-sysgen(:,1:2)-sysgen(:,3)*sysy2),inf) 
