...
%               M. Susca, 27.12.2018 % added antistabilizable solution
%                computation
%
...
if nargin < 6
    discr = false;
    options = struct('tol',0,'discr',discr);
else
    validateattributes(options,{'struct'},{'nonempty'},'','OPTIONS')
end

if isfield(options,'stable')
...
%     end
    ...
    ev = ordeig(At,Et); 
    select2 = select_spectrum(ev(i2),discr,smarg,stable,tola);
    select = [false(ninf,1); select2];
%     if discr
